...
export default function AdminPage() {
    ...
    switch (type) {
      case 'videos':
        videoForm.handleSubmit(async (data) => {
          ...
          // Se passou pela verificação, continuar com o cadastro
          if (!editingItem) {
            videoForm.setValue('videoUrl', `https://www.youtube.com/watch?v=${videoId}`);
          }

          // Buscar dados do vídeo do YouTube
          const videoDataResponse = await fetch(`/api/youtube/video/${videoId}`, {
            credentials: 'include',
          });
          ...
          if (!videoDataResponse.ok) {
            toast({
              variant: "destructive",
              title: "Erro",
              description: "Não foi possível buscar dados do vídeo",
            });
            return;
          }
          ...
          // Preencher formulário com dados do YouTube
          ...
          videoForm.setValue('duration', videoData.duration || '00:00:00');

          toast({
            title: "Dados do vídeo carregados!",
            description: `Vídeo: ${videoData.title}`,
          });
        } catch (error) {
          ...
          toast({
            variant: "destructive",
            title: "Erro",
            description: "Falha ao processar URL do YouTube",
          });
        }
      } else {
        // Limpar dados se URL for inválida
        if (!editingItem) {
          videoForm.setValue('title', '');
          videoForm.setValue('description', '');
          videoForm.setValue('thumbnailUrl', '');
          videoForm.setValue('duration', '');
        }
      }
    }
  };
  ...
  // Watch para mudanças na URL do vídeo (REMOVIDO O TRECHO DUPLICADO ABAIXO)
  useEffect(() => {
    const subscription = videoForm.watch((value, { name }) => {
      if (name === 'videoUrl' && value.videoUrl) {
        handleVideoUrlChange(value.videoUrl);
      }
    });
    return () => subscription.unsubscribe();
  }, [videoForm.watch]);

  // Handler para submissão do formulário de vídeo (CONTINUA AQUI)
  const onVideoSubmit = async (data: any) => {
    // Validação básica já passou pelo Zod

    // Se for edição, fazer update
    if (editingItem) {
      try {
        const response = await fetch(`/api/videos/${editingItem.id}`, {
          method: 'PUT',
        ...
        if (!response.ok) throw new Error('Falha ao atualizar vídeo');
        ...
        toast({
          variant: "destructive",
          title: "Erro",
          description: "Falha ao atualizar vídeo",
        });
      }
      return; // IMPORTANTE: Sair aqui para não executar o código de criação
    }

    // Se não for edição, continuar com o fluxo de criação normal...
    // (resto do código de criação continua aqui sem alterações)
                  return;
                }
              }
            } catch (error) {
