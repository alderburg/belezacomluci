...
async function createTables() {
  ...
  for (const sql of sqlStatements) {
    try {
      ...
      const tableName = sql.match(/CREATE TABLE IF NOT EXISTS (\w+)/)?.[1] || 'extension';
...
async function importData() {
  ...
  const tablesInOrder = [
    'users', 'subscriptions', 'categories', 'videos', 'products', 'coupons',
  ...
  for (const tableName of tablesInOrder) {
    ...
    for (const row of data) {
      try {
        ...
        // Ignora erros de duplicatas silenciosamente
      }
    }
    
    console.log(`✅ ${tableName} importado!`);
    totalImported += data.length;
  }
  
  console.log(`\n📈 Total importado: ${totalImported} registros\n`);
}

async function main() {
  try {
    console.log('🚀 Configuração completa do Railway\n');
    console.log('═══════════════════════════════════════\n');
    ...
    console.log('🎉 Seu banco Railway está pronto!\n');
